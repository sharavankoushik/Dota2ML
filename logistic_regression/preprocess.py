...
# row is a different match and each column is a features
...
NUM_MATCHES = match_collection.count()-500
...
for i, record in enumerate(match_collection.find().limit(NUM_MATCHES)):
    pbar.update(i)
    try:
        Y[i] = 1 if record['radiant_win'] else 0
        players = record['players']
    except:
         print(record['radian_win'])
