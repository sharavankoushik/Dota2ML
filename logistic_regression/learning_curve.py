...
from logistic_regression.train import train
...
def evaluate(model, X, Y, positive_class, negative_class):
    ...
    print("No of times the algorithm predicted correctly",
          correct_predictions)  # No of times the algorithm predicted correctly
...
def plot_learning_curves(num_points, X_train, Y_train, X_test, Y_test, positive_class=1, negative_class=0):
    ...
    for training_set_size in training_set_sizes:
        model = train(X_train, Y_train, training_set_size)
        print("Training Set No ", training_set_size)
    ...
    plt.plot(training_set_sizes, training_errors, 'r^-', label='Training accuracy')
    plt.plot(training_set_sizes, test_errors, 'bs-', label='Test accuracy')
    plt.ylabel('Accuracy')
    plt.xlabel('Number of training samples')
    plt.title('Logistic Regression Learning Curve')
...
def main():
    training_data = np.load('train_147615.npz')
